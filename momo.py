...
Original file is located at
    https://colab.research.google.com/drive/1w5p6LWnvDDGaki0sY--oXOoSYsfsMON6
"""
...
def analyze_sentiment(df, column):
    ...
    # .locを使って安全に列を更新
    df_copy = df.copy()
    sentiment_results = df_copy[column].astype(str).str[:512].apply(lambda x: analyzer(x)[0])
...
def clustering_texts(df, column, num_clusters=3, stop_words=None):
    ...
    tfidf_matrix = vectorizer.fit_transform(df_copy[column].astype(str))
...
with st.sidebar:
    st.header("⚙️ 設定")
    # GoogleスプレッドシートのURL入力からCSVアップロードに変更
    ...
    with col1:
        ...
        if 'sentiment_label' in df_sentiment.columns:
            fig_pie = px.pie(df_sentiment, names='sentiment_label', title='感情の割合',
                             color='sentiment_label', color_discrete_map={'POSITIVE':'#2ca02c', 'NEGATIVE':'#d62728'})
